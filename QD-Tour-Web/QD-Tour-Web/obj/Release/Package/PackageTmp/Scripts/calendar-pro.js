(function () {
    ...
    $.fn.extend({
        calendar: function (opt) {
            ...
            if ($(opt.ele).children().hasClass('ht-rili-querybox')) {
                $(opt.ele).children().remove();
            }
        ...
        calendarGetActive: function () {//获取当前选中日期的值
            ...
            if (activeEle.length == 2) {
                for (var i = 0; i < activeEle.length; i++) {
                    data.push({ date: activeEle[i].dataset.date, money: activeEle[i].innerText.split('$').pop() });
                }
            } else if (activeEle.length == 1) {
                data.push({ date: activeEle[0].dataset.date, money: activeEle[0].innerText.split('$').pop() });
            } //else {
            //    alert("请选择时间！");
            //    return;
            //}
